...
def get_item_price(args, item_code, ignore_party=False):
	...
	args['price_list'] = frappe.db.get_value("Item", item_code, "cpce_pricelist", cache=True)
...
def get_item_price_Purchase(args, item_code, ignore_party=False):
	"""
		Get name, price_list_rate from Item Price based on conditions
			Check if the desired qty is within the increment of the packing list.
		:param args: dict (or frappe._dict) with mandatory fields price_list, uom
	...
	return frappe.db.sql(""" select name, buying_regular_list_price, uom
		from `tabItem Price` {conditions}
		order by valid_from desc, batch_no desc, uom desc """.format(conditions=conditions), args)



def get_price_list_rate_for_Purchase(args, item_code):
	"""
	...
	item_price_data = 0
	price_list_rate = get_item_price_Purchase(item_price_args, item_code)
	if price_list_rate:
		...
		general_price_list_rate = get_item_price_Purchase(item_price_args, item_code,
			ignore_party=args.get("ignore_party"))

		if not general_price_list_rate and args.get("uom") != args.get("stock_uom"):
			item_price_args["uom"] = args.get("stock_uom")
			general_price_list_rate = get_item_price_Purchase(item_price_args, item_code, ignore_party=args.get("ignore_party"))
